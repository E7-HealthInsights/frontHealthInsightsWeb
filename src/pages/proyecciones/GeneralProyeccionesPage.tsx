...
import GeneralProyeccionDetalle from '../../components/features/projections/GeneralProyeccionDetalle'
...
export default function GeneralProyeccionesPage() {
  ...
  const saveMutation = useMutation({
    ...
    }: { titulo: string; descripcion: string; resultado: GeneralResultado }) =>
      saveProyeccion(titulo, descripcion, resultado as unknown as Parameters<typeof saveProyeccion>[2]),
    onSuccess: () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-hi-bg)]">
          ...
          <>
            <div className="mb-8">
              ...
              <p className="text-sm text-[var(--color-hi-text-sub)] mt-0.5">
                ¿Cómo evolucionará la diabetes en México? · {user?.name}
              </p>
            </div>
            ...
            {!isLoading && !isError && proyecciones.length === 0 && (
              <div className="flex flex-col items-center justify-center gap-4 py-24
                ...
                <p className="text-sm text-[var(--color-hi-text-hint)]">
                  Crea tu primer escenario con el botón +
                </p>
              </div>
            )}
          </>
      ...
      <GeneralProyeccionModal
        ...
        onSave={(titulo, descripcion, resultado) =>
          saveMutation.mutateAsync({ titulo, descripcion, resultado })
        }
      />
