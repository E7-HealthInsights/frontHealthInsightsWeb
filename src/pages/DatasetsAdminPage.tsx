...
function extractErrorMessage(err: unknown): string {
  if (err && typeof err === 'object' && 'response' in err) {
    ...
    if (status === 409) return 'Ya existe un dataset con ese nombre de archivo. Usa un archivo diferente.'
    if (status === 400) return 'Los datos enviados no son válidos. Revisa los campos e intenta de nuevo.'
    if (status === 500) return 'El servidor encontró un error al procesar el archivo. Verifica que el CSV esté bien formado y que los tipos de dato sean correctos para los valores de cada columna.'
  }
  if (err instanceof Error) return err.message
  return 'Error inesperado al subir el dataset. Intenta de nuevo.'
}
...
export default function DatasetsAdminPage() {
  ...
  const handleUpload = async (payload: UploadDatasetPayload) => {
    setUploadError('')
    try {
      const created = await uploadDataset(payload)
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 gap-3
            ...
            <svg width="44" height="44" viewBox="0 0 24 24" fill="none"
              ...
              strokeLinejoin="round" aria-hidden="true" className="opacity-30">
              <ellipse cx="12" cy="5" rx="9" ry="3" />
              <path d="M3 5v14c0 1.66 4.03 3 9 3s9-1.34 9-3V5" />
              <path d="M3 12c0 1.66 4.03 3 9 3s9-1.34 9-3" />
            </svg>
          ...
          /* Grid de cards — 2 columnas como en el Figma */
